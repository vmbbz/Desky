...
The **Open conversation** action is also main-owned and takes no renderer argument. A fixed provider-route registry may prefer an installed official client only when that exact OS protocol has been reviewed and Electron confirms a handler. OpenClaw is currently admitted through its documented credential-free `openclaw://chat` route. Provider/session identifiers, endpoints, prompt text, and secrets are never interpolated into the URI. Claude Agent SDK session ids are not assumed to be Claude Desktop conversation ids; Hermes documents shared sessions but no public session-opening URI; Codex documents no admitted desktop deep link. Those adapters therefore open Deskii's synchronized control-center transcript until their stable route contracts are proved. Missing handlers and launch failures also fall back to Deskii.
...
Voice is split into two mutually exclusive provider-neutral contracts rather than one ambiguous microphone toggle. Dictation opens an explicit user-gesture microphone stream and OpenClaw transcription-only Talk session. It converts exact 8 kHz mono samples to G.711 mu-law, updates the shared in-memory draft with partial/final transcript text, and never submits that text until the user presses Send. Full-duplex conversation opens a distinct `desky.voiceConversation` bridge and OpenClaw `realtime`/`gateway-relay`/`agent-consult` session. Routing is a provider contract: GPT-Live rejects `force-agent-consult`; the admitted OpenClaw source runtime prefers native Quicksilver delegation and falls back to the same Gateway-owned consult when a finalized user turn produces none. Other admitted providers may select forced consultation only if supported. The session negotiates mono PCM16 or G.711 input/output, dispatches microphone frames with monotonic media timestamps through an official-client-aligned four-send ownership bound, tolerates isolated append failures but closes after three consecutive failures, and releases local capture without waiting for stale acknowledgements during teardown. Sustained local microphone activity gives immediate Hearing feedback, but only provider partial/final transcript events own a semantic turn. Playback schedules at most eight seconds of decoded output, acknowledges provider marks only when playback reaches them, and maps audibly non-silent playback to the avatar's speaking state. Zero-amplitude transport/comfort frames are discarded until the first audible sample of a response, so keepalive media cannot invent a Speaking turn, consume the playback bound, or cancel a provider that is still preparing its answer. A short playback lead absorbs the Gateway's 20 ms network/IPC frame jitter, and a bounded output-gap grace keeps genuine output inside one stable Speaking phase. Natural barge-in uses guarded local speech detection to clear playback and cancel the exact output turn; the Gateway/provider `clear` event is the authoritative late-packet boundary.

The renderer owns a fail-closed output lifecycle rather than trusting one browser callback. `audio-done` or a final assistant transcript settles to Listening after the playback queue drains; a temporary frame gap settles to Thinking rather than falsely remaining Speaking. Provider `clear` and the internal explicit-cancellation fallback invalidate the exact output generation and suppress late scoped or unscoped packets until the next user turn. Explicit cancellation targets the owned output turn, not a newer partial-input turn. A wall-clock watchdog clears sources and timers if Web Audio never reports completion, attempts to resume a suspended output context when new audio arrives, and surfaces an output-device diagnostic. Stop/restart clears every turn id, ignored-turn marker, source, mark, gap timer, and watchdog, so voice and avatar state cannot leak into the next session.

Main owns both remote session identities, enforces one renderer owner and one voice path globally, validates every identifier/audio frame, and exposes only normalized transcript/audio/mark/clear/error/closed events. Bounded pre-start event buffers span runtime, IPC owner, and renderer listener registration so an immediate `ready` or provider rejection cannot disappear between the create response and UI ownership. OpenClaw credentials, ChatGPT account IDs, native Talk events, and provider tools never cross preload. Stop, Escape, surface destruction, adapter/session change, disconnect, permission failure, audio backlog, malformed output, and remote error all stop tracks, audio nodes, timers, and the remote session. Starting full-duplex voice replaces the text composer with a dedicated live-voice dock; text and voice are not presented as simultaneous interaction modes. Assistant transcripts remain bounded speech-bubble projections and auto-dismiss while the authoritative Gateway conversation remains available.

Capability discovery requires `talk.catalog`, the full Talk method set, `talk.event`, one configured realtime provider, `gateway-relay`, and admitted mono formats. Discovery proves configuration, not provider entitlement. A synchronous setup failure or asynchronous provider rejection downgrades the current full-duplex capability to `setup-required` until reconnect; a 403 recovery hint directs the user to verify the selected OAuth account, model, and model-specific voice. The same fail-closed rule applies separately to transcription provider setup. Codex, Hermes, Claude, and Simulation direct adapters remain unsupported in the current implementation. If OpenClaw itself consults a similarly named model, that remains an OpenClaw-owned speech and agent session; it is never presented as voice on the direct Deskii adapter of that name. The initial Microsoft Store profile also reports both voice contracts unsupported because its current generated MSIX manifest does not declare microphone access. Direct macOS carries a purpose string, but macOS hardware/permission evidence remains open. Device selection, truthful visemes, wake words, and background listening remain separate gates.

F5d.4 separates the future speech runtime from the agent runtime rather than cloning this controller inside every adapter. A main-owned `VoiceCoordinator` will compose either a coupled realtime speech runtime, such as OpenClaw Talk, or a cascade speech runtime that performs STT, sends the final transcript through the selected `AgentAdapterRuntime`, and synthesizes only its normalized user-visible assistant text. The cascade speech provider is deliberately unselected: the admitted Hermes `/v1` API Server reports audio unavailable, while Hermes's audio routes belong to a separate Dashboard/Desktop server that requires its own admission. Codex's current native realtime surface stays laboratory-only because the official source marks it experimental. Claude Agent SDK has no programmatic audio contract; interactive Claude Code dictation is unavailable with Deskii's API-key topology and supplies no assistant speech. Provider credentials remain main-owned and every non-OpenClaw provider remains voice-disabled until its exact pairing passes the full matrix. The accepted decision and gates are in `docs/adr/0012-provider-neutral-speech-plane.md` and `docs/research/VOICE-PROVIDER-TOPOLOGY-2026-09-01.md`.
