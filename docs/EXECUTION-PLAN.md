...
- **F5d.4 provider-neutral speech-plane decision complete:** ADR 0012 separates future speech runtimes from agent runtimes. After F5d.3 closes, extract the proven OpenClaw path behind one `VoiceCoordinator`; then admit Hermes's authenticated relay STT/streaming-TTS path and compose it with normalized Hermes/Codex agent adapters. Codex native realtime remains a version-pinned lab gate while experimental, and Claude uses cascade only after authenticated agent admission. No provider instruction-file edit is required. No external gateway, Python runtime, Whisper model, or Claude executable may enter the base artifact; release verification now fails on those payloads. Do not add wake words/background listening without a separate privacy/lifecycle gate. Evidence: `docs/verification/F5D4-SPEECH-PLANE-DISPOSITION-2026-09-01.md`.
