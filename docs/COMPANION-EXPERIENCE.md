...
`avatar-expression-controller.ts` is the separate capability-aware additive layer. It drives a deterministic bounded blink schedule, restrained look-at, and small available-preset expressions, restores authored values on disposal, neutralizes gaze for reduced motion, and becomes a no-op when an avatar lacks the relevant feature. Agent-originated semantic action commands, richer gesture selection, speech visemes, licensed external clips, and binary VRM compatibility evidence remain open and must preserve the same ownership boundary.
