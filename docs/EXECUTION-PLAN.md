...
- **F5d.3 playback hardening implemented:** the first packaged microphone exercises exposed 20 ms frame-boundary phase thrash, a missing Web Audio completion escape, incomplete stale-packet reset, text-only completion that could remain Thinking, leading comfort-silence misclassified as a failed assistant response, and a reference launcher whose inherited terminal pipe could later crash Electron with `EPIPE`. The renderer now uses a bounded jitter lead and gap grace, audible-sample phase admission, terminal transcript/audio settlement, exact cancelled-turn suppression, suspended-context recovery, and a queue-duration watchdog. Leading zero-amplitude transport frames are discarded without cancellation. The GUI tolerates only broken optional output pipes, and the packaged voice launcher starts with independent standard handles. Five controller regressions, two pipe regressions, and privacy-bounded evidence-recorder tests cover the corrected contracts. Evidence: `docs/verification/F5D3-VOICE-PLAYBACK-HARDENING-2026-09-01.md`.
- **F5d.3 next:** the first post-fix 180-second package observation proved a clean launch/capture lifecycle with no `EPIPE` or visual-exercise error but contained no user voice activity, so it is not assistant-audio evidence. Repeat the real packaged microphone allow/deny and prove audible assistant output, then record user/assistant transcript ordering, Interrupt during output, clear/mark ordering, provider disconnect and same-session recovery. Add reviewed input/output-device selection and measured audio-driven expression/viseme truth afterward. Direct Hermes/Codex/Claude adapters remain voice-unsupported in the current implementation; an OpenClaw session consulting a similarly named model is still OpenClaw-owned and must not be mislabeled.
