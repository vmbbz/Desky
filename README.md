...
F3c.2 now restores and clamps the companion per display arrangement, flips and shifts the anchored bubble at work-area edges, uses measured avatar bounds for its deliberate character hit target, passes transparent-region clicks through, and provides session-only full click-through with tray, shortcut, and control-center recovery. Always-on-top is now a saved user control. Contextual composer behavior and the broader packaged Windows/macOS device matrix remain next.
