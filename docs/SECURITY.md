...
- `talk.catalog` readiness is treated only as configuration evidence. A later provider rejection disables realtime starts until reconnect and preserves a redacted bounded diagnostic; Deskiii does not loop on a known 403 or infer its cause. GPT-Live 403 responses are overloaded, so recovery must verify the selected account, model, and model-specific voice before making an entitlement claim.
- GPT-Live agent consultation remains Gateway-owned. Deskiii leaves unsupported forced routing unset and never promotes assistant acknowledgement text into an agent result. The admitted OpenClaw source runtime prefers native delegation and uses its existing bounded, authenticated agent-consult runner only when a finalized user turn produces none; it bounds appended context and suppresses duplicate native delivery.
