...
This is now a deployed but deliberately non-payable HTTPS testnet boundary, not yet a durable operational commerce service. The wallet shell and Functions exist, and the PostgreSQL adapter/migration pass local integration tests, but deployment still requires an admitted PostgreSQL service, real identity-provider PKCE flow, managed signing-key custody, backups/restore drills, rate limits, reconciliation workers, monitoring, incident evidence, and clean-device hardware verification. Every payment provider remains unreachable. No checkout or paid UI is exposed by Electron.
