...
The renderer serializes a bounded capture queue and stops instead of silently dropping microphone audio. Output is decoded into a bounded Web Audio schedule; leading zero-amplitude transport frames are ignored until real audible output begins, while silence inside an established audible response retains its timing. Silence alone never cancels the remote session. Clear/cancel stops already scheduled sources, playback marks are acknowledged at their audible boundary, and avatar `speaking` follows scheduled audible audio rather than transcript arrival. The live-voice dock replaces the text composer for the session and exposes phase, Interrupt when meaningful, End, Escape, and reduced-motion-safe activity. No wake word, launch-time capture, persistence, or background listener is admitted.
