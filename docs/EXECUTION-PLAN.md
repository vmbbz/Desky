...
- **F5d.2 live status:** current official OpenClaw and Codex `2026.8.1` packages are installed, the Runner Courage OpenAI profile has been refreshed through official OAuth, and the stable local Gateway is healthy on the saved Deskiii endpoint. `talk.catalog` advertises `gpt-live-1-codex`, the separate GPT-Live Codex V3 voice set, Gateway relay, agent consultation, PCM16/G.711, and barge-in. Replacing stale GA voice `cedar` with GPT-Live voice `spruce` resolves the earlier overloaded 403; a real session now admits with PCM16 24 kHz. No microphone or assistant audio was sent by the bounded CLI probe.
