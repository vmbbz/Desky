...
The first executable profile contract is now admitted. All four current catalog revisions (the three free companions and the isolated Toothpaste pilot revision) name `desky-humanoid-standard-v1`; the external Seed-san engineering fixture names the same profile without becoming a product offer. The profile is a file asset, not a filename list in scheduler code. Runtime proves exact state/program ownership, required bones, reviewed intensity and forbidden root motion before exposing programs to the controller. The bundled presentation catalog advanced to revision 3 so older descriptors cannot silently omit the profile identity.
...
The control center now has a first-class Companions route backed by a strict bundled catalog and free-entitlement decision. F4.2 admits Milk, CoolBanana, and Astronaut from pinned Open Source Avatars registry commit `0f9a1b2fd99894736563d55b2c9dc9125700d081`. Each exact model and thumbnail revision has a byte length, SHA-256, canonical source-record hash, source timestamp, embedded-usage review, and immutable product/revision ID. The cards use brokered hash-pinned artwork and declare commerce/payment rails disabled.

Selection is transactional. Main downloads and validates the exact admitted model into the content-addressed cache, exposes it to the ambient runtime as a pending revision, and persists the new active/fallback pair only after Three.js/three-vrm, humanoid, embedded-rights, texture, and motion setup succeed. Failure clears the pending revision and reloads the previous working companion. Restart reads the committed revision and verified cache; a visual-test-only network-denial mode proves offline cache reuse.

F4.3 adds a selection-neutral 3D detail preview, a 256 MiB catalog-aware LRU pass that protects active/fallback/pending revisions, packaged corrupt-sidecar reacquisition, and a repeatable switch-soak harness. Forty consecutive packaged switches completed without a stale or partial transaction; deep VRM disposal reduced post-warm-up growth between the separate twenty- and forty-switch runs, but a longer reference-device GPU plateau test remains a release-performance gate.

F4.4 exposes a typed cache inventory and safe model-removal command. The UI distinguishes verified offline, missing/online-only, corrupt/repair-needed, and protected records; displays physical storage against the fixed ceiling; and states that removal does not revoke the companion. Acquisition is protected before its verification await, closing the activation/removal race. Packaged proof previewed and removed Astronaut, reduced physical storage, and left Milk protected. Separate packaged restarts repaired deliberately corrupted provenance for Milk and Astronaut, completing the three-avatar corrupt-sidecar matrix. F4.8 closes real VRM 1.0 engineering compatibility with the external VPL-licensed Seed-san fixture. The remaining marketplace M2 work is a product-suitable CC0 VRM 1.0 revision, Mac packaging, and longer lifecycle evidence—not x402.

F3d.2 closes the schema/runtime portion of the per-avatar animation-profile design. A clean packaged Windows run downloaded and transactionally activated all three free revisions across six switches with no exercise error; all model/cache records were verified and every committed activation passed renderer admission. Structural VRM 0.x/1.0 binding still covers all 85 canonical clips, while only the profile's three ambient programs and Jump enter executable runtime policy. The eventual product-suitable CC0 VRM 1.0 revision still needs its own final visual approval; the engineering fixture does not substitute for that product decision.
