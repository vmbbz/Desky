...
Next: close F5d.3 now that the refreshed Cosy Chiruka OAuth profile is first, `gpt-live-1-codex`/`spruce` session admission passes, and the packaged exercise's frame-jitter, stuck-Speaking, reset, comfort-silence, and launcher-pipe defects are fixed behind regressions and a rebuilt Windows-direct package. The reference OpenClaw runtime now uses its own per-user Scheduled Task rather than a foreground automation process; it remains external to Deskiii. A post-fix 180-second observation completed without an `EPIPE` or visual-exercise error, but no voice session was entered and it is not counted as microphone/output proof. Re-prove real audible assistant audio first, then transcript/output ordering, barge-in during audible playback, clear/mark timing, provider disconnect and same-session recovery. F5d.4 then extracts the proven path behind one provider-neutral speech plane and selects an independent cascade speech runtime for direct Hermes/Codex agents. The admitted Hermes `/v1` server reports audio unavailable; its Dashboard/Desktop audio routes are a separate admission decision. Codex native realtime stays experimental/lab-only. Claude enters cascade only after authenticated agent admission and cannot reuse its Claude.ai-only interactive dictation in the API-key SDK topology. Then add reviewed device selection and truthful audio-driven facial response. Remote OpenClaw/Hermes deployment evidence, product-suitable CC0 VRM 1.0 selection, Windows external full-screen/manual accessibility/physical-display evidence, macOS, Claude, and production commerce remain owner/external gates.
