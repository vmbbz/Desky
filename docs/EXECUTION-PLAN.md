...
- Prefer only installed, documented provider-client routes; fall back to Deskii for unadmitted, absent, or failed routes.
...
- **F5d.2 interaction closure implemented and refined in F5d.3:** entering voice replaces the text composer in both ambient and Control Center surfaces with a dedicated Listening/Hearing/Thinking/Speaking dock, state activity, End and Escape. Natural interruption uses the official-client two-frame speech guard, immediately clears local playback, and invokes exact-turn Gateway cancellation; provider `clear` owns the late-packet boundary. There is no ordinary interrupt button. Assistant voice text uses the existing bounded auto-dismiss bubble. Voice mode does not expose a disabled Send field or imply simultaneous text ownership.
- **F5d.2 live status:** the dated foundation used official OpenClaw/Codex `2026.8.1`. `talk.catalog` advertised `gpt-live-1-codex`, the separate Codex V3 voice set, Gateway relay, agent consultation, PCM16/G.711, and barge-in. Replacing stale GA voice `cedar` with `spruce` resolved the earlier overloaded 403 and admitted PCM16 24 kHz. The operator later exhausted and switched the reference account. The replacement profile has now been reauthenticated and read-only verified; catalog and functional roundtrip claims must still be re-proved rather than inherited from the old snapshot.
- **F5d.3 playback hardening implemented and normal completion proved:** the first packaged microphone exercises exposed 20 ms frame-boundary phase thrash, a missing Web Audio completion escape, incomplete stale-packet reset, text-only completion that could remain Thinking, leading comfort-silence misclassified as a failed assistant response, a continuous post-response comfort tail, and a reference launcher whose inherited terminal pipe could later crash Electron with `EPIPE`. The renderer now uses a bounded jitter lead and gap grace, audible-sample phase admission, terminal transcript/audio settlement, exact cancelled-turn suppression, suspended-context recovery, a queue-duration watchdog, and a 500 ms consecutive-silence ceiling after audible output. Leading zero-amplitude transport frames are discarded without cancellation. The GUI tolerates only broken optional output pipes, and the packaged voice launcher starts with independent standard handles. Packaged OAuth evidence now proves non-silent microphone input, non-silent GPT-Live output, final user/assistant transcripts, two responses in one session, `Speaking -> Thinking -> Listening`, and bubble dismissal without a visual-exercise error. Evidence: `docs/verification/F5D3-VOICE-PLAYBACK-HARDENING-2026-09-01.md`.
- **F5d.3 routing and natural-interruption correction implemented:** the reference GPT-Live topology is `brain=agent-consult` with `consultRouting` unset because current OpenClaw rejects forced routing for `gpt-live-1-codex`. OpenClaw source commits `21a29a5f0` and `0988619cc` add native-first fallback and prevent a late native duplicate from aborting fallback work already in flight. This corrects the observed skills failure: `GPT-Live delegation superseded`, followed by inactive-harness cleanup. It is distinct from the older OpenClaw 2026.8.1 stale-request-root admission defect; `/startupz` monitors only global starting/draining state, retains cleanup controls, and restores connected state if the same socket survives recovery. The ordinary Interrupt button is removed. Two sustained speech frames clear only local playback and continue upstream microphone delivery; GPT-Live owns interruption from incoming audio. Deskii does not call the relay-closing cancellation RPC for ordinary speech barge-in. A partial user transcript enters `Hearing you`, and a 12-second non-finalization watchdog ends the failed session cleanly instead of hanging. Focused tests and isolated lint/type gates pass; account reauthentication and corrected Gateway restart are complete. Live proof still awaits a real three-skills result, natural barge-in, and second same-session result. Direct Hermes/Codex/Claude adapters remain voice-unsupported in the current implementation; an OpenClaw session consulting a similarly named model is still OpenClaw-owned and must not be mislabeled.
- **F5d.4 provider-neutral speech-plane decision corrected:** ADR 0012 separates future speech runtimes from agent runtimes. After F5d.3 closes, extract the proven OpenClaw path behind one `VoiceCoordinator`; then select and admit an independent cascade speech runtime before composing it with Hermes/Codex and later Claude. The current Hermes `/v1` server explicitly reports audio unavailable; its Dashboard/Desktop audio routes are a separate, unadmitted server. Codex native realtime remains a version-pinned lab gate while experimental. Claude's interactive dictation cannot serve the API-key Agent SDK and provides no speech output. No provider instruction-file edit is required. No external gateway, Python runtime, Whisper model, or Claude executable may enter the base artifact; release verification now fails on those payloads. Do not add wake words/background listening without a separate privacy/lifecycle gate. Evidence: `docs/verification/F5D4-SPEECH-PLANE-DISPOSITION-2026-09-01.md` and `docs/verification/F5D4A-PROVIDER-VOICE-ADMISSION-CORRECTION-2026-09-01.md`.
