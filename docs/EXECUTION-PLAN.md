...
- **F5b.1/F5b.2 complete; F5b.3 resilience complete:** the pinned Hermes `0.20.5` API Server passes bearer admission, disposable sessions, OAuth-backed `gpt-5.4` streaming, terminal dedupe, guarded-command approval denial/allow-once, Stop during actual execution, passive idle recovery, active transport recovery, no replay, and a fresh post-recovery model turn. A real installed Windows gateway restart also passes after the bounded retry schedule was calibrated to its measured service lifecycle. Hermes remains unregistered and `production: false` until durable credential/rotation, packaged UI lifecycle, remote TLS/Store, typed-action discovery, and macOS gates pass.
