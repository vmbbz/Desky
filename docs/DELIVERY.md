...
F5a.1 is implemented: the executable generic contracts, active-runtime registry, `desky:adapter:*` IPC/preload surface, and provider-neutral renderer state now sit above the unchanged OpenClaw transport host. OpenClaw validates its opaque configuration and maps native gateway/session/run state behind `OpenClawRuntime`; provider-native frames and native state names do not reach the renderer. Registry tests cover enumeration, full command routing, switching teardown, inactive-runtime isolation, and renderer-safe errors. F5a.2 is active: the bounded Codex stdio peer, main-owned executable discovery, exact version admission, reproducible canonical schema manifest, consumed-field validators, semantic mapper, opaque workspace grants, read-only-first sandbox disclosure, unregistered fixture-conformant runtime, bounded full-readmission reconnect, shell-free process-tree termination and awaited application shutdown are implemented. Typed actions are truthfully unsupported because the only client-local registration surface is experimental. The authenticated source-runtime matrix now passes real model streaming, write approval deny/allow, actual descendant cancellation, same-session recovery, process crash/reconnect and post-crash streaming. It found and closed a serious gap where `turn/interrupt` reported cancellation without stopping the command. Direct-profile registry/provider selection and repetition through the packaged UI boundary remain before SDK stabilization.
