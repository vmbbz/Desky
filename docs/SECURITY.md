...
The Codex adapter is direct-profile only. It accepts no renderer-provided executable path: main scans its PATH for the expected filename, resolves a real absolute file, reads its version without a shell, and currently admits only `codex-cli 0.146.0-alpha.3`, matching the locally generated schema baseline. The process receives a reviewed environment allowlist that deliberately excludes ambient API keys and unrelated secrets. App-server launches with `shell: false` and a fixed argument vector; every JSONL message is limited to 1 MiB; stderr retention is limited to 64 KiB and renderer diagnostics are redacted; correlated requests time out; malformed/oversized output and unexpected exit fail closed. Native reasoning, commands, output, diffs, arguments, and paths are not forwarded. The current runtime is not registered as selectable; process-tree termination, generated-schema automation/coverage, workspace picker consent, reconnect, and packaged model/tool/approval tests remain admission gates.
