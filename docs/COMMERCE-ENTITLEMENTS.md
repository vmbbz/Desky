...
## Implemented foundation — F4x.1a–F4x.2d, 2026-08-25
...
This is not yet a deployed durable network service. The executable API/repository/recovery/checkout/browser boundaries now pass local adversarial tests, but production still requires the hosted database adapter and migrations, real identity-provider PKCE flow, TLS/domain, managed signing-key custody, separately built/deployed accessible wallet page, CSP asset pipeline, backups/restore drills, rate limits, reconciliation workers, monitoring, incident evidence, and clean-device hardware verification. Every payment provider remains unreachable. No checkout or paid UI is exposed by Electron.
