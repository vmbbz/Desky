...
- **F5a.2 in progress:** supervised stdio JSONL, direct-only truthful descriptor, main-owned executable discovery, exact CLI admission, reviewed environment allowlist, and an unregistered runtime are complete. The reproducible schema gate now canonicalizes and verifies all 273 non-experimental CLI-generated JSON schemas against a metadata-only manifest, with individual pins for every consumed schema; safe projection validators cover all native fields used in state transitions and malformed consumed notifications close the runtime. Fixture conformance covers sessions, streaming, paired tools, command/file approvals, cancellation, terminal deduplication, disconnect, and process/protocol failure. Live installed-CLI initialization/account/thread-list smoke passes without a model turn. Next: approved workspace/sandbox UI, bounded restart/reconnect, stable typed-action discovery, then packaged authenticated model/tool/approval/cancellation tests before registry admission.
