...
- **F5a.2 in progress:** supervised stdio JSONL, direct-only truthful descriptor, main-owned executable discovery, exact CLI admission, reviewed environment allowlist, and an unregistered runtime are complete. The reproducible schema gate canonicalizes and verifies all 273 non-experimental CLI-generated JSON schemas against a metadata-only manifest; safe projection validators cover every native field used in state transitions. Main-owned workspace consent now uses short-lived opaque grants, canonical revalidation, native write confirmation, read-only-first disclosure, no root/home write scope, and no renderer-supplied path. The Codex form is compiled but unreachable while the runtime remains unregistered. Fixture conformance covers sessions, streaming, paired tools, command/file approvals, cancellation, terminal deduplication, disconnect, and process/protocol failure. Live installed-CLI initialization/account/thread-list smoke passes without a model turn. Next: bounded restart/reconnect and process-tree evidence, stable typed-action discovery, then packaged authenticated model/tool/approval/cancellation tests before registry admission.
