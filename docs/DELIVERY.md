...
F5b.1 Hermes foundation is implemented against official source revision `057dcdf236f8a6a26721c10fcc6ccb72726e272a`: authenticated capability and version admission, strict endpoint policy, bounded structured HTTP/SSE, sessions, streaming, tool/subagent lifecycle, per-run approvals, truthful stop, disconnect, redaction, and exactly-once terminal fixtures pass. F5b.2 now passes both executable opt-in source-runtime matrices against loopback Hermes Agent `0.20.5`. Wrong-bearer redaction, authenticated source/version/capability admission, and disposable session resources are real-server proved. Hermes's `openai-codex` provider is OAuth-authorized with `gpt-5.4`; real assistant streaming, terminal dedupe, guarded-command approval denial, allow-once, and Stop during actual execution all pass. The adapter stays unregistered and `production: false`; automatic reconnect, crash/version-drift recovery, vault-grade credential persistence, clean packaged lifecycle, remote TLS/Store eligibility, and typed Desky-action discovery remain open.

F5b.2 evidence is recorded in `docs/verification/F5B2-HERMES-AUTHENTICATED-MATRIX-2026-08-24.md`.
...
3. Complete Hermes automatic reconnect, crash/version-drift, credential persistence, packaged lifecycle, and typed-action discovery gates, then return to the Claude Agent SDK authenticated matrix. Keep both unadmitted until their complete source and packaged matrices pass; keep commerce capability separate.
