...
- **F5d.2 live status:** the official OpenClaw checkout at `8ac6379011c20702875e5219268b40969c5b90b2` validates the realtime configuration and advertises a ready OpenAI gateway relay with barge-in. The intended runner OAuth profile is first, unexpired at test time, and contains a 36-character account id. Session negotiation reaches the provider bridge, but `/v1/live` rejects it with 403. Deskiii now converts that asynchronous rejection into `setup-required` until reconnect. Live assistant audio/barge-in proof remains blocked by external GPT-Live account access, not by missing Gateway methods or Deskiii credential selection.
- **F5d.3 next:** after GPT-Live access is available, record the real microphone allow/deny, user/assistant transcript, audible output, Interrupt during output, clear/mark ordering, provider disconnect and same-session recovery matrix. Then add reviewed input/output-device selection and measured audio-driven expression/viseme truth. Do not add wake words/background listening or claim Hermes/Codex/Claude voice until their native transports or an independently reviewed local voice runtime exist.
