...
- **F5d.3 playback hardening implemented:** the first packaged microphone exercise exposed 20 ms frame-boundary phase thrash, a missing Web Audio completion escape, incomplete stale-packet reset, and text-only completion that could remain Thinking. The renderer now uses a bounded jitter lead and gap grace, audible-sample phase admission, terminal transcript/audio settlement, exact cancelled-turn suppression, suspended-context recovery, and a queue-duration watchdog. Four controller regressions prove continuous phase, watchdog recovery, clear/reset with late scoped and unscoped packets, and no-audio response completion. Evidence: `docs/verification/F5D3-VOICE-PLAYBACK-HARDENING-2026-09-01.md`.
- **F5d.3 next:** repeat the real packaged microphone allow/deny and prove audible assistant output after this playback fix, then record user/assistant transcript ordering, Interrupt during output, clear/mark ordering, provider disconnect and same-session recovery. Add reviewed input/output-device selection and measured audio-driven expression/viseme truth afterward. Direct Hermes/Codex/Claude adapters remain voice-unsupported in the current implementation; an OpenClaw session consulting a similarly named model is still OpenClaw-owned and must not be mislabeled.
