...
Voice is split into two mutually exclusive provider-neutral contracts rather than one ambiguous microphone toggle. Dictation opens an explicit user-gesture microphone stream and OpenClaw transcription-only Talk session. It converts exact 8 kHz mono samples to G.711 mu-law, updates the shared in-memory draft with partial/final transcript text, and never submits that text until the user presses Send. Full-duplex conversation opens a distinct `desky.voiceConversation` bridge and OpenClaw `realtime`/`gateway-relay`/`agent-consult` session. It negotiates mono PCM16 or G.711 input/output, serializes a bounded capture queue, schedules at most eight seconds of decoded output, acknowledges provider marks only when playback reaches them, and maps audibly non-silent playback to the avatar's speaking state. Zero-amplitude transport/comfort frames are discarded until the first audible sample of a response, so keepalive media cannot invent a Speaking turn, consume the playback bound, or cancel a provider that is still preparing its answer. A short playback lead absorbs the Gateway's 20 ms network/IPC frame jitter, and a bounded output-gap grace keeps genuine output inside one stable Speaking phase. Turn-scoped barge-in clears local output and asks the Gateway to cancel only the current realtime response.

The renderer owns a fail-closed output lifecycle rather than trusting one browser callback. `audio-done` or a final assistant transcript settles to Listening after the playback queue drains; a temporary frame gap settles to Thinking rather than falsely remaining Speaking. Interrupt and provider `clear` invalidate the exact output generation and suppress late scoped or unscoped packets until the next user turn. A wall-clock watchdog clears sources and timers if Web Audio never reports completion, attempts to resume a suspended output context when new audio arrives, and surfaces an output-device diagnostic. Stop/restart clears every turn id, ignored-turn marker, source, mark, gap timer, and watchdog, so voice and avatar state cannot leak into the next session.
