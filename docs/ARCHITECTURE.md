...
Voice input is a separate provider-neutral capability, not text-command guessing or an always-on listener. In the first admitted slice, an explicit user click opens one microphone stream and OpenClaw transcription-only Talk session. The renderer captures audio with echo cancellation, noise suppression, and automatic gain control, converts exact 8 kHz mono samples to G.711 mu-law, and serializes bounded chunks through `desky.voiceInput`. Main owns the remote session identity and validates every chunk against its one renderer owner; it exposes only normalized partial/final transcript events. OpenClaw credentials and native Talk events never cross preload. Stop, Escape, surface destruction, adapter disconnect, permission failure, audio backlog, and remote error all stop tracks and close or discard the session. The transcript updates the shared in-memory draft and is never sent to the agent until the user presses Send.

The negotiated capability is initially available only when the Gateway advertises all of `talk.session.create`, `talk.session.appendAudio`, `talk.session.close`, and `talk.event`. Method discovery cannot prove that a provider credential is usable. If session creation reports a missing or unconfigured transcription provider, main downgrades the live capability to `setup-required`, supplies bounded provider-neutral recovery guidance, and disables further capture until reconnect. Other adapters remain explicitly unsupported until they expose an admitted streaming-transcription contract. The initial Microsoft Store profile also reports voice unsupported because its current generated MSIX manifest does not declare microphone access; this avoids a UI promise that the package cannot satisfy. Direct macOS carries a purpose string, but macOS hardware/permission evidence remains open. Full-duplex provider audio, TTS, barge-in, voice selection, wake words, and background listening are separate future gates.
