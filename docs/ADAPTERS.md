...
- Remaining admission sequence: register Codex only in the direct profile, add explicit provider selection, and repeat the authenticated matrix through the packaged UI/lifecycle boundary. Source-level authenticated behavior, CLI/schema mismatch fixtures, Windows package creation and ordinary packaged lifecycle are complete. The Codex consent form remains compiled but unreachable until that admission round. WebSocket is excluded because the official current surface labels it experimental and unsupported for production.
