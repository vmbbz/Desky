...
- The system browser opens only an exact same-origin `/checkout/{sessionId}` URL with no credential, query, or fragment. Browser-launch failure preserves the admitted session for explicit reopen; it does not recreate approval or payment state.
