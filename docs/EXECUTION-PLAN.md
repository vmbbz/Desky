...
Status: F4x.1a–c now provide strict commerce contracts and transactional conformance plus the hosted API/repository boundary, rotating Ed25519 JWKS, OS-encrypted refresh rotation, PKCE/idempotent clean-device recovery, signed bounded offline lease, pinned offline verification key, trusted-time checks, and reconciliation-before-persist. F4x.2a–d pin the provider-disabled Base Sepolia x402 v2 exact/EIP-3009 boundary and secure browser handoff. F4x.2e.1–e.3 deploy the isolated Netlify/Supabase boundary, private schema/runtime role, SSL/CA enforcement, multi-instance dispatch exclusion, Supabase-subject identity, transactional free grants, authenticated identity/quote/checkout/recovery routes, digest-only rotation, shared rate limits, operations, verified encrypted restore and monitoring. F4x.2e.4 admits exact CC0 Toothpaste provenance and the fixed 0.10 test-USDC offer, plus an independent three-confirmation Base observer and atomic paid-grant closure. The public testnet RPC is deployed; `/healthz` is live while payment readiness remains fail-closed because merchant/offer are intentionally absent. Next: pass a real Supabase app-user exchange, bind the owner-provided dedicated merchant address, fund the payer with test USDC, execute the complete funded/recovery matrix, then replace pilot RPC/log-only monitoring with production-grade redundant infrastructure before any mainnet decision.
