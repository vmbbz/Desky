...
F3a now includes runtime VRM 0.x/1.0 capability inspection, core-humanoid validation, embedded usage-permission conflict checks, SHA-256 avatar provenance, a deterministic offline multi-clip FBX converter, an avatar-neutral canonical clip format, target-VRM runtime binding, and a strict manifest for reviewed animation conversions. The F3b motion runtime maps every normalized state to one deterministic full-body owner, admits only provenance-matching reviewed clips, and otherwise uses restrained procedural poses. Its first CC0 candidate library generates 84 canonical motions from the free Quaternius Standard inventories, with state bindings and weighted multi-step action/ambient programs stored in validated asset metadata rather than animation-name code. A typed live-only agent-action lane and OpenClaw tool package can request Wave/Jump without parsing model text or changing user prompts; Jump resolves through reviewed files and Wave keeps a procedural fallback. Packaged visual admission, live plugin evidence, representative binary VRM suite, persistent asset sidecars, truthful speech visemes, and the full state/performance matrix remain open.

F3c now provides the actual avatar-first desktop companion: the resting surface is Milk plus a compact launcher, meaningful responses appear in an anchored light bubble, and connection/session management stays in a separate control center. Milk remains continuously alive and selects bounded, non-repeating idle programs from rights-reviewed file metadata; every decorative program yields to meaningful agent state. Dragging the character moves and persists the clamped native companion; Shift/Alt drag, scrolling, or arrow keys rotate the 3D view. Measured avatar bounds define the deliberate hit target, transparent regions pass clicks, and tray, shortcut, and control-center recovery protect full click-through. The composer appears only after explicit interaction, `Escape` preserves its session-only shared draft, Stop remains reachable, long responses route to the control center, and a late-opening window recovers the current response or approval from one main-owned snapshot. This focused behavior is package-verified on Windows, including a live OpenClaw assistant stream through `openai/gpt-5.6-sol`. Avatar catalog/model downloads now cross a bounded main-process broker rather than relying on sandboxed renderer CORS.
