...
2. Implement F5d.4a speech-plane extraction without behavior drift, then F5d.4b Hermes relay STT/streaming-TTS and F5d.4c cross-agent cascade for Hermes/Codex. Keep Codex native realtime lab-only while experimental; Claude waits for authenticated agent admission and its optional-pack/hosted distribution decision.
