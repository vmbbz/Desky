...
- Admission status: the opt-in source-runtime matrices pass against a loopback, bearer-authenticated Hermes Agent `0.20.5` installed from the pinned revision. The provider-independent matrix proves wrong-bearer rejection/redaction, exact server topology/version/capability admission, and disposable session create/delete. The authenticated matrix uses Hermes's `openai-codex` provider with OAuth-authorized `gpt-5.4` and proves real assistant streaming, exactly one terminal, approval denial, allow-once, Stop during actual command execution, idle and active transport recovery, no replay, a fresh post-recovery turn, and recovery across the installed Windows gateway's real process restart. This does not admit the adapter for release: the descriptor remains `production: false`, direct-only, and unregistered; durable credential storage/rotation, packaged UI lifecycle, remote TLS/Store eligibility, typed agent actions, and macOS evidence remain open.
...
Hermes is prioritized through its authenticated API-server resources and run-event SSE contract; Claude follows through the supported Agent SDK in direct builds. Hermes now has authenticated streaming, approval, cancellation, and resilience evidence but remains unadmitted until its credential, packaged lifecycle, remote, typed-action, and cross-platform gates pass. Neither integration may scrape terminal presentation text, inherit consumer login without explicit upstream permission, or become `production: true` before its complete authenticated source and packaged matrices pass. The desktop companion can proceed independently because it consumes only normalized `AdapterEvent` values.
