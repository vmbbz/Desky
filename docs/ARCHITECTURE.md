...
The negotiated capability is available only when the Gateway advertises all of `talk.session.create`, `talk.session.appendAudio`, `talk.session.close`, and `talk.event`. Other adapters remain explicitly unsupported until they expose an admitted streaming-transcription contract. The initial Microsoft Store profile also reports voice unsupported because its current generated MSIX manifest does not declare microphone access; this avoids a UI promise that the package cannot satisfy. Direct macOS carries a purpose string, but macOS hardware/permission evidence remains open. Full-duplex provider audio, TTS, barge-in, voice selection, wake words, and background listening are separate future gates.
