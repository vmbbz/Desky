...
F4x.2e.1 adds a separately built Netlify checkout workspace, hardened static wallet shell, five fixed Functions, PostgreSQL migration, and an asynchronous transactional `PostgresCheckoutLedger`. The unfunded testnet shell is live at `https://desky-checkout-testnet.netlify.app`; missing durable database and payment configuration fail closed with sanitized 503 responses. Netlify's current team plan refused built-in database provisioning, so no service state, merchant, facilitator, paid IPC, funded payment, or production provider is enabled. ADR 0008 records the deployment split and blocker.
