...
- **F5a.2 in progress:** supervised stdio JSONL, direct-only truthful descriptor, main-owned executable discovery, exact CLI/schema-baseline admission, reviewed environment allowlist, bounded native validators, thread/turn/item/approval normalization, and an unregistered runtime are complete. Fixture conformance covers sessions, streaming, paired tools, command/file approvals, cancellation, terminal deduplication, disconnect, and process failure. Live installed-CLI initialization/account/thread-list smoke passes without a model turn. Next: committed generated-schema pipeline and broader native validation, approved workspace/sandbox UI, restart/reconnect, stable typed-action discovery, then packaged authenticated model/tool/approval/cancellation tests before registry admission.
