...
A scheduled Netlify Function evaluates migration state, pending orders, indeterminate settlement count, and maximum reconciliation age every fifteen minutes and writes structured severity events to immutable deploy logs. It also closes bounded expired quote-only orders and expired checkout sessions only while they remain `ready` or `awaiting-wallet`; signing and settlement states are never swept. This is pilot monitoring, not a production paging system.

## Consequences

- Electron continues to verify Desky JWKS, access JWT, offline lease, installation, the bounded set of catalog revisions, and exact grants before persisting refresh state. Legacy singular-catalog access tokens are normalized only during their short rollover lifetime.
...
- The first owner-approved Base Sepolia funded success and restoration lifecycle are proved. Mainnet, broad selling regions, Store commerce, production RPC/paging, and production key custody remain disabled.
