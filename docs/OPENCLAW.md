...
Natural interruption is speech-driven. Deskiii keeps echo-cancelled microphone capture active while output plays and mirrors the official OpenClaw desktop guard: RMS at least `0.02`, peak at least `0.08`, for two consecutive capture frames. Sustained speech immediately clears local playback and sends turn-scoped `talk.session.cancelOutput` with reason `barge-in`; the Gateway/provider `clear` event remains the authoritative boundary for rejecting late audio. The live dock intentionally contains phase, activity, and End only—there is no ordinary Interrupt button. The same RPC remains available internally for playback overflow and bounded recovery.
