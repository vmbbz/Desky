...
Status: F4x.1a–c now provide strict commerce contracts and transactional conformance plus the hosted API/repository boundary, rotating Ed25519 JWKS, OS-encrypted refresh rotation, PKCE/idempotent clean-device recovery, signed bounded offline lease, pinned offline verification key, trusted-time checks, and reconciliation-before-persist. F4x.2a–d pin the provider-disabled Base Sepolia x402 v2 exact/EIP-3009 boundary and secure browser handoff. F4x.2e.1 adds a separate Netlify wallet build and live unfunded HTTPS shell. F4x.2e.2 provisions the dedicated Supabase PostgreSQL boundary, private schema/runtime role, SSL/CA enforcement, canonical migration, and real multi-instance dispatch exclusion. `/healthz` is live; payment readiness remains fail-closed and no provider is admitted. Next: rotate the disclosed owner credential, add authenticated identity/quote/session/recovery routes and reconciliation operations, prove backup/restore and monitoring, then execute the funded Base Sepolia and clean-device restoration matrix.
