...
Status: F4x.1a–c now provide strict commerce contracts and transactional conformance plus the hosted API/repository boundary, rotating Ed25519 JWKS, OS-encrypted refresh rotation, PKCE/idempotent clean-device recovery, signed bounded offline lease, pinned offline verification key, trusted-time checks, and reconciliation-before-persist. F4x.2a–c pin the provider-disabled Base Sepolia x402 v2 exact/EIP-3009 boundary; add durable authorization, monotonic settlement observations, transaction uniqueness, retry/closure blocking, and settled-evidence-only grant; and establish strict hosted checkout session routes, a recoverable system-browser handoff, ephemeral signature processing, and a durable one-shot `/settle` dispatch claim. These modules remain unwired from Electron IPC and paid providers remain disabled. Remaining F4x.1 operations: deploy the real database/API/identity/TLS/key-custody stack, migrations/backups/restore, reconciliation workers/monitoring, multi-instance tests, and clean-device hardware evidence. Next F4x.2 work is the hosted wallet-page adapter and funded Base Sepolia success/rejection/expiry/replay/timeout/callback-loss/restart matrix; no production commerce is enabled.
