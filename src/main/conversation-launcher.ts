...
const providerConversationRoutes: Readonly<Record<string, ProviderConversationRoute>> = Object.freeze({
  ...
  // The client owns its session picker; Deskiii never serializes gateway secrets
 ...
 * admitted here. Missing, unadmitted, or failed clients fall back to Deskiii's
...
export class ConversationLauncher {
  ...
  async open(): Promise<ConversationOpenResult> {
    ...
    if (!route) return this.openDeskiii(state.adapterId, 'no-admitted-provider-route');

    const handlerName = this.dependencies.getProtocolHandlerName(route.protocolUrl).trim();
    if (!handlerName) return this.openDeskiii(state.adapterId, 'provider-client-not-installed');
    ...
    try {
      ...
      return this.openDeskiii(state.adapterId, 'provider-client-launch-failed');
    }
  }
  ...
  private openDeskiii(
    ...
    this.dependencies.openDeskiii();
    return { destination: 'deskiii', reason, adapterId };
  }
}
