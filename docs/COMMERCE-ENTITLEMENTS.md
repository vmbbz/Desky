...
- F4x.2e.2–e.3 connect the hosted boundary to the dedicated Supabase project and implement the issuer/operations side: external Supabase subject proof, opaque Desky identity, three transactional free grants, Ed25519 JWKS/access/offline issuance, digest-only recovery/refresh rotation, optional authoritative paid-offer quotes, shared rate limits, operator reconciliation visibility, encrypted logical backup/isolated restore and scheduled structured monitoring. The real successful Supabase-user exchange, paid offer, merchant/facilitator, automated unknown-settlement observer, external paging and funded payment remain open. ADRs 0009–0010 and `docs/verification/F4X2E3-IDENTITY-OPERATIONS-2026-08-25.md` hold the evidence.
...
This is now a deployed but deliberately non-payable HTTPS testnet operations boundary. PostgreSQL, signing-key custody, fixed identity/session/quote routes, digest-only recovery, shared rate limits, operator queue, encrypted backup/restore and scheduled monitoring exist. It is not production commerce: no real Supabase user exchange has passed, no paid offer/merchant/facilitator is configured, unknown settlement has no admitted automated status observer, external paging/off-device escrow are absent, and no funded transaction has run. Every payment provider remains unreachable from Electron.
