...
- The full matrix is deliberately opt-in because Hermes tools execute on the API-server host. Its probes are bounded to a non-mutating print command and a sleeping Python process, and every temporary Desky session is deleted after the test. The bearer is read only from the process environment and is never printed.
- Admission status: the provider-independent source matrix passes against a loopback, bearer-authenticated Hermes Agent `0.20.5` installed from the pinned revision: wrong-bearer rejection/redaction, exact server topology/version/capability admission, and disposable session create/delete are live-proved. No Hermes model provider is authorized on this machine, so real-model streaming, approval, and execution cancellation remain unrun rather than simulated. The descriptor therefore remains `production: false`, direct-only, and unregistered; `reconnect` and agent actions are still truthfully unsupported. The authenticated full-model source/package matrices, Store eligibility, and automatic reconnect are later gates.
