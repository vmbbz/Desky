...
Admitted model and thumbnail caching is now main-owned and content-addressed. Exact bytes and their provenance sidecars are written through temporary files and renamed into separate object/record paths. Every read revalidates revision identity, pinned registry commit and record hash, byte length, SHA-256, media signature/VRM envelope, licence, and source URL. Missing or invalid pairs are reacquired only through the bounded broker. Valid cached active assets load without network access. Bounded eviction is still open and must never remove the active or rollback revision.
