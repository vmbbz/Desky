...
- Remaining admission sequence: packaged authenticated model stream, tool approval allow/deny, cancellation-during-tool, reconnect/crash, CLI/schema-mismatch and clean-package lifecycle matrix; then registry/provider selection admission. The Codex consent form is compiled but unreachable while the runtime remains unregistered. WebSocket is excluded because the official current surface labels it experimental and unsupported for production.
