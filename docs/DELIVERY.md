...
Next: finish F5d.3 interruption/recovery now that the refreshed Cosy Chiruka OAuth profile is first, `gpt-live-1-codex`/`spruce` session admission passes, and the packaged exercise's frame-jitter, stuck-Speaking, reset, leading/trailing comfort-silence, and launcher-pipe defects are fixed behind regressions and a rebuilt Windows-direct package. The reference OpenClaw runtime uses its own per-user Scheduled Task rather than a foreground automation process; it remains external to Deskiii. Packaged OAuth evidence now proves non-silent microphone input, non-silent assistant output, final user/assistant transcripts, two responses in one session, `Speaking -> Thinking -> Listening`, and bubble dismissal. Next prove barge-in during audible playback plus a second post-interrupt turn, then clear/mark timing, provider disconnect/reconnect, permission denial, and device removal. F5d.4 then extracts the proven path behind one provider-neutral speech plane and selects an independent cascade speech runtime for direct Hermes/Codex agents. The admitted Hermes `/v1` server reports audio unavailable; its Dashboard/Desktop audio routes are a separate admission decision. Codex native realtime stays experimental/lab-only. Claude enters cascade only after authenticated agent admission and cannot reuse its Claude.ai-only interactive dictation in the API-key SDK topology. Then add reviewed device selection and truthful audio-driven facial response. Remote OpenClaw/Hermes deployment evidence, product-suitable CC0 VRM 1.0 selection, Windows external full-screen/manual accessibility/physical-display evidence, macOS, Claude, and production commerce remain owner/external gates.
...
1. Complete the remaining F5d.3 packaged Windows-direct realtime evidence now that OpenClaw GPT-Live OAuth normal input/output and completion pass: Interrupt during audible output, post-interrupt same-session recovery, permission deny, clear/mark order, backlog/error, disconnect/reconnect, surface close, and device removal. Streaming dictation remains separately blocked by the missing transcription API-key profile; it must not be conflated with the admitted GPT-Live realtime path.
