...
The schema and strict runtime parser live in `src/shared/asset-provenance.ts`. The renderer computes SHA-256 from the exact brokered avatar bytes and attaches the validated record to the loaded scene for diagnostics. F4 persists admitted model and thumbnail objects under their exact SHA-256 with a separate parsed provenance sidecar. Both files are written through same-directory temporary files, and every cache read rechecks admission revision, pinned registry commit/record hash, byte length, SHA-256, media envelope, source, licence, and provenance before use. A corrupt or incomplete pair is replaced only from the exact admitted URL; the active avatar is not changed until renderer admission succeeds. Cache access updates only the validated record timestamp. A 256 MiB catalog-aware least-recently-used pass removes exact non-protected record/object paths and retains shared objects while referenced; active, rollback, acquisition, and pending revisions are always protected. The user-facing removal command removes only the exact optional model record/object. It deliberately retains the thumbnail, product, source, licence, entitlement, and selected-companion history.
...
`src/renderer/avatar/vrm-capabilities.ts` implements the version/capability and embedded-usage review boundary. The runtime now consumes the advertised blink, look-at, and preset-expression subset through a separate additive controller and no-ops when a feature is absent. Speech visemes remain disabled until a truthful timing source is implemented. Structural fixtures cover both versions, and F4.8 adds an external, hash-pinned real Seed-san VRM 1.0 compatibility fixture under the VRM Public License 1.0. It passes production loading, cache/offline repair, normalized state motion, Jump, framing, reduced motion, and WebGL recovery without being bundled or added to the marketplace. The final product-suitable CC0 VRM 1.0 catalog revision and every-enabled-clip visual matrix remain open.
...
Avatar-to-animation admission is also explicit. Each admitted avatar revision carries `animationProfileId`; the current `desky-humanoid-standard-v1` profile is a separate reviewed JSON asset. It requires the 15 core retarget bones, binds exactly the four state modes and four runnable programs in the built-in library, assigns autonomous intensity, and forbids root motion. Production loading rejects an unknown profile, missing required bone, library/profile runnable-policy drift, or root-motion drift before playback. Catalog-only animation data is not registered with the runtime controller.
