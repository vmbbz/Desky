...
Status: F4x.1a–c now provide strict commerce contracts and transactional conformance plus the hosted API/repository boundary, rotating Ed25519 JWKS, OS-encrypted refresh rotation, PKCE/idempotent clean-device recovery, signed bounded offline lease, pinned offline verification key, trusted-time checks, and reconciliation-before-persist. F4x.2a–d pin the provider-disabled Base Sepolia x402 v2 exact/EIP-3009 boundary and secure browser handoff. F4x.2e.1 adds a separate Netlify wallet build, fixed Functions, PostgreSQL migration/adapter, and a live unfunded HTTPS shell. The current Netlify team plan rejected managed-database creation, so health/readiness/payment routes fail closed and no provider is admitted. Next: provision an approved PostgreSQL service, apply migration 0001, run multi-instance tests, add identity/quote/session routes and reconciliation operations, then execute the funded Base Sepolia and clean-device restoration matrix.
