...
- Replaced the hard-coded control-center action with an argument-free main-owned conversation router. Installed OpenClaw Companion is preferred through its documented fixed `openclaw://chat` link; unsupported, absent, and failed clients fall back to Deskii.
- Kept Claude, Hermes, and Codex on the safe fallback because their current Deskii session identities have no proved stable desktop-app conversation route.
- Repaired release/reference tooling after the Deskii product-name migration so artifact verification resolves the current product directory instead of silently inspecting a stale `Desky-*` package.
...
Next: finish F5d.3 interruption/recovery on the reauthenticated selected OpenClaw account and corrected source Gateway. The packaged exercise's frame-jitter, stuck-Speaking, reset, leading/trailing comfort-silence, launcher-pipe, and evidence-finalization defects are fixed behind regressions. The reference OpenClaw runtime remains external to Deskii and uses its own per-user Scheduled Task. Deskii's same-origin `/startupz` monitor covers only the Gateway's global lifecycle: explicit starting/draining states remove the green claim, retain already-admitted Stop/approval/voice-cleanup controls, continue probing, and restore connected state if the same authenticated socket survives a return to `started`. It cannot detect the older OpenClaw 2026.8.1 stale-request-root admission bug while `/startupz` says `started`; current OpenClaw source corrects that defect. A separate packaged run then finalized a skills request under healthy startup state, but GPT-Live emitted only an acknowledgement and no native delegation or agent run. OpenClaw source commit `21a29a5f0` now prefers native delegation and, after a 250 ms grace, falls back to the same bounded Gateway-owned consult with event-order and duplicate suppression. Its 54 owner/lifecycle tests pass with one Bun-only skip; isolated changed-file lint and OpenAI-extension typecheck pass. The ordinary Interrupt button remains removed: two sustained speech frames at RMS `0.02` and peak `0.08` clear local playback and request exact-turn `barge-in`, while provider/Gateway `clear` rejects late packets. A partial utterance enters `Hearing you` and the 12-second finalization watchdog prevents hanging. Reauthentication and patched Gateway restart are complete; now prove a real three-skills result, natural barge-in during audible playback, and a second same-session result before mark timing, provider disconnect/reconnect, permission denial, and device removal. F5d.4 then extracts the proven path behind one provider-neutral speech plane and selects an independent cascade speech runtime for direct Hermes/Codex agents. Claude still awaits its authenticated agent gate. Remote deployments, product-suitable CC0 VRM 1.0 selection, external Windows evidence, macOS, Claude, and production commerce remain owner/external gates.
...
1. Complete the remaining F5d.3 packaged Windows-direct matrix on the reauthenticated OAuth profile and restarted corrected source Gateway: a real three-skills agent result through native-first/fallback consultation, natural speech barge-in during audible output, second same-session result, permission deny, clear/mark order, backlog/error, disconnect/reconnect, surface close, and device removal. Streaming dictation remains separately blocked by the missing transcription API-key profile; it must not be conflated with the admitted GPT-Live realtime path.
2. Implement F5d.4a speech-plane extraction without behavior drift, then F5d.4b selects and admits one independent cascade speech topology before F5d.4c composes it with Hermes/Codex. A Hermes `/v1` audio extension is preferred; its separate Dashboard/Desktop server requires a full second-server admission and is not assumed. Keep Codex native realtime lab-only while experimental; Claude waits for authenticated agent admission and its optional-pack/hosted distribution decision.
3. Add the reviewed microphone declaration and privacy copy only when promoting voice into the Microsoft Store profile; until then Store capability and controls remain disabled.
4. Supply the Microsoft Partner Center identity, legal publisher name, Windows code-signing certificate, privacy/support/security URLs and source licence; then run the same makers in production mode on a protected clean signing worker.
5. Prove a signed direct `0.1.0 -> 0.1.1` update through the hosted HTTPS feed and repeat install/update/uninstall on a clean Windows account/device; run final Partner Center/WACK preflight with the reserved Store identity.
6. Deploy a trusted operator-owned OpenClaw/Hermes ingress and run the full authenticated lifecycle through it. Keep typed actions unsupported and the MCP helper deferred under the one-app decision. Hermes remains excluded from Store builds.
7. Complete the remaining `<3%` Windows idle margin plus external full-screen, manual accessibility, physical display reconnect, multi-monitor, virtual-desktop, final CC0 VRM 1.0, and macOS evidence.
8. Keep the completed Base Sepolia pilot frozen except for optional manual rejection evidence and production operations hardening; mainnet and Store commerce remain disabled. Owner-deferred lanes remain macOS/Keychain and Claude's authenticated matrix.
