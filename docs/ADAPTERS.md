...
- Admission truth: WebSocket `hello-ok` is necessary but not sufficient for a green connection. Main also observes OpenClaw's same-origin, credential-free `/startupz` lifecycle endpoint. Explicit `starting`/`draining` states withdraw new-work admission while preserving the existing transport solely for already-admitted cancellation, approval resolution, and session cleanup. Unknown or unavailable lifecycle responses remain compatible with older protocol-v4 Gateways.
...
OpenClaw is the first mapping: `talk.catalog` must report a configured ready realtime provider, `gateway-relay`, and compatible PCM16 or G.711 formats; Deskii then requests `talk.session.create({ sessionKey, mode: "realtime", transport: "gateway-relay", brain: "agent-consult" })`. Routing remains provider-specific. Current `gpt-live-1-codex` must leave `consultRouting` unset because OpenClaw rejects forced routing for that model. The admitted OpenClaw source runtime is native-first: provider delegation wins, while a finalized user turn with no native delegation starts the same Gateway-owned bounded agent consult after a short grace and returns it as speakable session context. Deskii neither parses an acknowledgement nor runs a second agent. A different realtime provider may opt into `force-agent-consult` only when its own catalog/runtime contract supports it. Audio append, provider clear, turn-scoped output cancellation, playback marks, close, and `talk.event` remain main-owned. The renderer receives a generated session id plus negotiated formats, not Gateway/provider credentials or tool authority.

Method/catalog readiness is not an entitlement claim. A synchronous creation error or asynchronous provider rejection disables further starts on that connection and publishes `setup-required`. The actual bounded provider error is still shown for diagnosis. A previously authenticated reference profile admitted a real `gpt-live-1-codex`/`spruce` session with PCM16 24 kHz after correcting the stale GA voice selection. Packaged Windows-direct evidence proves non-silent microphone input, non-silent assistant audio, user/assistant transcripts, normal completion, and a second turn in one session. The operator later changed accounts after exhausting that profile's usage. The selected profile has now been reauthenticated and the corrected source Gateway restarted; the real three-skills/fallback and natural-barge-in roundtrip still must be re-proved before the next F5d.3 claim.

The renderer serializes a bounded capture queue and stops instead of silently dropping microphone audio. Output is decoded into a bounded Web Audio schedule; leading zero-amplitude transport frames are ignored until real audible output begins, while up to 500 ms of consecutive silence preserves ordinary pauses inside an established response and longer continuous comfort tails are discarded. Silence alone never cancels the remote session. A provider-acknowledged partial user transcript enters a distinct `Hearing you` phase and starts a 12-second finalization watchdog. A provider that never finalizes the utterance is cancelled with a recoverable error instead of hanging indefinitely; a final user transcript or real audible response cancels that watchdog. During playback, two consecutive microphone frames above the official-client RMS `0.02` and peak `0.08` thresholds trigger local clear plus exact-output-turn `talk.session.cancelOutput(reason=barge-in)`. Noise or one isolated frame does not cancel. The Gateway/provider `clear` remains authoritative for suppressing late packets, and no UI button is required. Playback marks are acknowledged at their audible boundary, and avatar `speaking` follows scheduled audible audio rather than transcript arrival. The live-voice dock replaces the text composer for the session and exposes phase, End, Escape, and reduced-motion-safe activity. No wake word, launch-time capture, persistence, or background listener is admitted.
...
3. Select and admit an independent cascade speech runtime. The current Hermes `/v1` API Server reports both audio capabilities false; Hermes Dashboard/Desktop audio is a second server surface and may be considered only with separate endpoint, authentication, version, TLS, and lifecycle admission. Deskii will not call `/api/audio/voice-config`, because it can return resolved provider credentials.
...
6. Add Claude to the cascade only after its authenticated agent/package gate. Interactive Claude Code dictation requires Claude.ai login and is unavailable to the API-key Agent SDK topology, so it is not a Deskii speech transport.
...
- **Claude:** Claude Desktop officially supports `claude://` chat and Code links, but Deskii's Claude Agent SDK session id is not a Claude.ai conversation id. No mapping is guessed, so Claude falls back to Deskii.
- **Hermes:** Hermes Desktop officially shares backend sessions and exposes session search, but no stable public conversation-opening URI is documented. Its MCP-install deep link is unrelated and is not reused. Hermes falls back to Deskii.
- **Codex:** no stable documented desktop conversation deep link is admitted. Codex falls back to Deskii.
