...
F5d.4 separates the future speech runtime from the agent runtime rather than cloning this controller inside every adapter. A main-owned `VoiceCoordinator` will compose either a coupled realtime speech runtime, such as OpenClaw Talk, or a cascade speech runtime that performs STT, sends the final transcript through the selected `AgentAdapterRuntime`, and synthesizes only its normalized user-visible assistant text. Hermes's authenticated audio relay is the first cascade candidate. Codex's current native realtime surface stays laboratory-only because the pinned official source marks it experimental, while Claude Agent SDK has no documented audio transport and must use the cascade. Provider credentials remain main-owned; Deskiii will not fetch Hermes's resolved provider credential for client-direct calls. The accepted decision and gates are in `docs/adr/0012-provider-neutral-speech-plane.md` and `docs/research/VOICE-PROVIDER-TOPOLOGY-2026-09-01.md`.
