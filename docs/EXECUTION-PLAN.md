...
Status: F4x.1a–c now provide strict commerce contracts and transactional conformance plus the hosted API/repository boundary, rotating Ed25519 JWKS, OS-encrypted refresh rotation, PKCE/idempotent clean-device recovery, signed bounded offline lease, pinned offline verification key, trusted-time checks, and reconciliation-before-persist. F4x.2a has begun with the pinned, provider-disabled Base Sepolia x402 v2 exact/EIP-3009 contract and strict facilitator boundary. All paid providers and checkout routes remain unreachable. Remaining F4x.1 operations: deploy the real database/API/identity/TLS/key-custody stack, migrations/backups/restore, reconciliation workers/monitoring, multi-instance tests, and clean-device hardware evidence. Next F4x.2 work adds durable uncertain/settled evidence and reconciliation before any funded testnet transaction; no production commerce is enabled.
