...
- **F5d.1 live status:** the packaged reference-device microphone-allow path reaches the authenticated OpenClaw session boundary. The current Gateway has OAuth but no transcription API-key profile, so it returns no registered transcription provider. Deskiii now downgrades this to `setup-required` instead of repeatedly advertising readiness. A configured credential is still required to record permission deny, partial/final, cancellation, backlog/error, disconnect, surface destruction, edit-before-send and fresh-install behavior.
