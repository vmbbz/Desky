...
The first rights-approved candidate library supplies 84 canonical Quaternius Standard clips. State bindings, action/catalog programs, exact steps, tags, quiet windows, and cooldowns live in the generated asset file; the scheduler contains no animation filename list. The daily surface uses an authored folded-arm idle continuously in both idle and disconnected modes, with weighted Search/Interact, Formal Walk, and rare Dance Break programs. The old `Idle_No_Loop`/`Yes` cadence is no longer autonomous because it read as constant head shaking and thumbs-up, and Crouch/Search was rejected on Milk for limb distortion. Rich sit/chat, magic, phone, crouch, and other edge cases remain catalog-only for future contextual requests and per-avatar visual admission. Authored steps play for their full canonical duration, adjacent steps crossfade without passing through the bind pose, and the final pose blends into the accepted state rather than snapping. Jump uses a two-file sequence; Wave stays procedural until a reviewed Wave asset exists. Speech visemes and representative binary VRM evidence remain open and must preserve the same ownership boundary.
