...
- Deskii never opens the microphone on launch, connection, an agent event, or a model request. Capture starts only from an enabled user-gesture button and ends on Stop/cancel, failure, disconnect, surface destruction, or permission loss.
- Electron's permission check and request handlers default-deny. They admit only `media`/`audio` from a known Deskii ambient or control-center main frame at the exact development entry or packaged `desky://app/main_window/index.html`; camera, display capture, subframes, foreign origins, and unknown media types remain denied.
...
- Audio is streamed and not persisted. The shared draft contains transcript text only in memory; it remains user-editable and requires the ordinary explicit Send action. Deskii does not silently submit dictation.
...
- Barge-in is scoped to the active realtime session and exact owned output turn. Two consecutive speech-like frames are required before the renderer clears local playback and requests cancellation; a single spike cannot cancel. The request cannot target an unrelated text run, newer input turn, or another adapter. Adapter/session changes and renderer destruction close the creating runtime's session before ownership is released.
- `talk.catalog` readiness is treated only as configuration evidence. A later provider rejection disables realtime starts until reconnect and preserves a redacted bounded diagnostic; Deskii does not loop on a known 403 or infer its cause. GPT-Live 403 responses are overloaded, so recovery must verify the selected account, model, and model-specific voice before making an entitlement claim.
- GPT-Live agent consultation remains Gateway-owned. Deskii leaves unsupported forced routing unset and never promotes assistant acknowledgement text into an agent result. The admitted OpenClaw source runtime prefers native delegation and uses its existing bounded, authenticated agent-consult runner only when a finalized user turn produces none; it bounds appended context and suppresses duplicate native delivery.
- The active full-duplex UI replaces the text composer with explicit Listening/Hearing/Thinking/Speaking state and End. Ordinary interruption is speech-driven through guarded local VAD and the provider's advertised cancellation/clear contract; no prominent cancellation button is required. Playback-overflow and bounded recovery use the same validated turn-scoped RPC with a distinct reason.
...
- Release capability is a compile-time authority. Webpack embeds one exact schema-v1 release manifest and selects a profile-specific runtime module graph. Main validates exact keys, profile/platform invariants, Store adapter cardinality, local-process policy, and commerce mode before startup. Missing, unknown, widened, or mismatched manifests fail closed; `DESKY_DISTRIBUTION` and remote flags are not authorities. Artifact policy inspects the packaged ASAR rather than trusting source-level conditionals.
