...
- Remaining admission sequence: typed actions only through a supported stable tool surface or a truthful unsupported decision; registry/provider selection admission; packaged authenticated model/tool/approval/cancellation/crash matrix. The Codex consent form is compiled but unreachable while the runtime remains unregistered. WebSocket is excluded because the official current surface labels it experimental and unsupported for production.
