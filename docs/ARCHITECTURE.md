...
F4x.2a–c pin the provider-disabled Base Sepolia x402 v2 exact/EIP-3009 boundary, make indeterminate settlement durable, and establish a non-custodial hosted checkout handoff. Verification produces immutable authorization evidence, never a transaction claim. Append-only observations monotonically reconcile timeout/callback loss through unknown/pending to settled or failed, with transaction reuse blocked across authorizations. Granting requires one exact durable settled observation; unknown/pending/settled-but-ungranted orders cannot close or retry. Main can coordinate a single-use exact-terms human approval with a short-lived hosted session and an exact system-browser URL; the service rechecks account, installation, order, quote, and canonical terms. Before `/settle`, the ledger atomically grants only one durable dispatch claim, so crash/replay cannot automatically broadcast twice. ADR 0005 records reconciliation and ADR 0006 records checkout/custody. No live wallet page, paid IPC, funded payment, or production provider is enabled.
