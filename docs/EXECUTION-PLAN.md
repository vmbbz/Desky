...
- **F5a.2 in progress:** supervised stdio JSONL, direct-only truthful descriptor, main-owned executable discovery, exact CLI admission, reviewed environment allowlist, and an unregistered runtime are complete. The reproducible schema gate canonicalizes and verifies all 273 non-experimental CLI-generated JSON schemas against a metadata-only manifest; safe projection validators cover every native field used in state transitions. Main-owned workspace consent uses short-lived opaque grants, canonical revalidation, native write confirmation, read-only-first disclosure, no root/home write scope, and no renderer-supplied path. Bounded restart/reconnect now repeats full admission up to three times, safely resumes a listed thread, expires approvals and never replays a lost turn; protocol/termination failures remain terminal. Shell-free OS process-tree teardown and an awaited bounded Electron quit barrier passed unit and real Windows parent/descendant evidence. Typed actions remain truthfully unsupported because client-local `dynamicTools` is experimental. The opt-in authenticated matrix passes real streaming, allow/deny approvals, actual tool-process cancellation, same-session recovery, unexpected crash/reconnect and post-crash streaming. It exposed and fixed a `turn/interrupt` containment gap by making Stop recycle the entire supervised tree. Next: direct-profile registry/provider-picker admission, followed by repetition through the packaged UI boundary.
