...
Status: F4x.1a–c now provide strict commerce contracts and transactional conformance plus the hosted API/repository boundary, rotating Ed25519 JWKS, OS-encrypted refresh rotation, PKCE/idempotent clean-device recovery, signed bounded offline lease, pinned offline verification key, trusted-time checks, and reconciliation-before-persist. F4x.2a–d pin the provider-disabled Base Sepolia x402 v2 exact/EIP-3009 boundary and secure browser handoff. F4x.2e.1–e.2 deploy the isolated Netlify/Supabase boundary, private schema/runtime role, SSL/CA enforcement, canonical migrations, and real multi-instance dispatch exclusion. F4x.2e.3 deploys Supabase-subject authentication, opaque Desky identity, transactional free grants, Ed25519 issuance/JWKS, authenticated identity/quote/checkout/recovery routes, digest-only rotation, shared rate limits, operator reconciliation visibility, verified encrypted backup/restore and scheduled structured monitoring. `/healthz` is live; payment readiness remains fail-closed and no paid offer/provider is admitted. Next: rotate the disclosed owner credential, pass a real Supabase-user exchange, choose the capped pilot product/price/regions and merchant/facilitator, add an admitted provider-status or Base-observer reconciliation contract, then execute the funded Base Sepolia and clean-device restoration matrix.
