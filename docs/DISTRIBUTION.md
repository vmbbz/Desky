...
F6a replaces the mutable `DESKY_DISTRIBUTION` helper with a build-time embedded and runtime-validated release manifest. Ordinary `npm run package` is explicitly `development-direct`; release candidates must use `package:windows:store-free`, `package:windows:direct`, or the corresponding macOS profile on macOS. Unknown profiles and platform mismatches fail before packaging. Main derives distribution and commerce solely from the embedded manifest and exposes the exact profile ID in diagnostics; runtime environment variables cannot widen it.
...
Dictation and full-duplex voice are currently enabled only by the direct profile after their separate runtime capability negotiations. The existing generated MSIX does not declare the privacy-sensitive microphone device capability, so Store-profile state deliberately projects both as unsupported even if its OpenClaw Gateway offers Talk. Enabling either in a Store release requires a reviewed custom manifest containing the microphone declaration, accurate privacy/Store disclosures, WACK and real packaged permission allow/deny evidence. Direct macOS packages embed `NSMicrophoneUsageDescription`; Mac App Store audio-input entitlement and hardware evidence remain open. This is a signed build-profile boundary, not a hidden renderer switch.
...
The 2026-09-01 Windows-direct voice baseline produces a 148,328,448-byte setup executable, a 147,282,790-byte full update, and a 13,197,736-byte ASAR. OpenClaw, Hermes, Python, Whisper/CTranslate2, native speech models, and the Claude executable are not bundled. Windows artifact verification now also rejects those external-runtime payload signatures from ordinary package roots. Voice uses Electron Web Audio and existing network/WebSocket facilities; adding a new voice mapping is not permission to add a local ML/runtime payload.

Claude is the deliberate exception to the otherwise small external-adapter bridges: the admission-only platform executable is 337,745,056 bytes. It remains excluded from Store-free and ordinary direct packages and cannot fit the current base-artifact budget. A future Claude release must use a separately signed optional direct provider pack or a reviewed hosted topology, with an independent size/licence/SBOM/update decision. Voice itself does not require that executable and must not be used to justify silently inflating the base installer.
