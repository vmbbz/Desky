...
The first rights-approved candidate library now supplies 84 canonical Quaternius Standard clips. State bindings, action/catalog programs, exact steps, tags, quiet windows, cooldowns, and exact cycle slots live in the generated asset file. The scheduler contains no animation filename list. The daily idle surface runs a strict two-look/one-celebration cadence in both idle and disconnected modes: `Idle_No_Loop` repeats to 7.5 seconds for each long look-around slot, and `Yes` supplies the 2.5-second celebration slot. Rich locomotion, sit/chat, magic, dance, phone, crouch, and search edge cases remain catalog-only for future contextual requests rather than firing randomly. Authored steps play for their full canonical duration, adjacent steps crossfade without passing through the bind pose, and the final pose blends into the accepted state rather than snapping. Jump uses a two-file sequence; Wave stays procedural until a reviewed Wave asset exists. Full packaged visual admission, speech visemes, and representative binary VRM evidence remain open and must preserve the same ownership boundary.
