...
Status: F4x.1a–c now provide strict commerce contracts and transactional conformance plus the hosted API/repository boundary, rotating Ed25519 JWKS, OS-encrypted refresh rotation, PKCE/idempotent clean-device recovery, signed bounded offline lease, pinned offline verification key, trusted-time checks, and reconciliation-before-persist. F4x.2a–b pin the provider-disabled Base Sepolia x402 v2 exact/EIP-3009 boundary and add durable authorization, unknown/pending/settled/failed observations, monotonic reconciliation, transaction uniqueness, retry/closure blocking, and settled-evidence-only grant. All paid providers and checkout routes remain unreachable. Remaining F4x.1 operations: deploy the real database/API/identity/TLS/key-custody stack, migrations/backups/restore, reconciliation workers/monitoring, multi-instance tests, and clean-device hardware evidence. Next F4x.2 work is the hosted checkout/browser-wallet handoff and funded Base Sepolia matrix; no production commerce is enabled.
