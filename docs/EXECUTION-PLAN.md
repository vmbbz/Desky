...
- **F5d.3 next:** record Interrupt during audible output plus a second same-session turn, then clear/mark ordering, provider disconnect/reconnect, permission denial, and input/output device removal. Add reviewed device selection and measured audio-driven expression/viseme truth afterward. Direct Hermes/Codex/Claude adapters remain voice-unsupported in the current implementation; an OpenClaw session consulting a similarly named model is still OpenClaw-owned and must not be mislabeled.
