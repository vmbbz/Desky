...
F5b.1 Hermes foundation is implemented against official source revision `057dcdf236f8a6a26721c10fcc6ccb72726e272a`: authenticated capability and version admission, strict endpoint policy, bounded structured HTTP/SSE, sessions, streaming, tool/subagent lifecycle, per-run approvals, truthful stop, disconnect, redaction, and exactly-once terminal fixtures pass. F5b.2 passes both executable opt-in source-runtime matrices against loopback Hermes Agent `0.20.5`. Wrong-bearer redaction, authenticated source/version/capability admission, disposable session resources, OAuth-backed `gpt-5.4` streaming, terminal dedupe, guarded-command approval denial/allow-once, and Stop during actual execution are real-server proved. F5b.3 adds a passive idle health check and three-attempt full re-admission with strict transient-error classification, approval expiry, exactly-one lost-turn failure, selected-session restoration, and no input replay. Fixture gates cover retry exhaustion, explicit-disconnect races, malformed events, corrupt responses, capability rejection, and version/model drift. The authenticated relay matrix passes idle loss, active SSE loss, and a fresh post-recovery model turn; the separately gated installed Windows service restart also passes. The adapter stays unregistered and `production: false`; vault-grade credential persistence/rotation, packaged UI lifecycle, remote TLS/Store eligibility, typed Desky-action discovery, and macOS evidence remain open.

F5b.2/F5b.3 evidence is recorded in `docs/verification/F5B2-HERMES-AUTHENTICATED-MATRIX-2026-08-24.md` and `docs/verification/F5B3-HERMES-RESILIENCE-2026-08-24.md`.
...
3. Complete Hermes credential persistence/rotation, packaged UI lifecycle, remote TLS/Store disposition, typed-action discovery, and macOS gates, then return to the Claude Agent SDK authenticated matrix. Keep both unadmitted until their complete source and packaged matrices pass; keep commerce capability separate.
