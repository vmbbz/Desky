...
- **F5d.3 routing and natural-interruption correction implemented:** the reference GPT-Live topology is `brain=agent-consult` with `consultRouting` unset because current OpenClaw rejects forced routing for `gpt-live-1-codex`. OpenClaw source commit `21a29a5f0` is native-first but no longer trusts native delegation to appear on every finalized user turn: after a 250 ms grace it runs the same Gateway-owned bounded agent consult and returns speakable session context, while native events win and duplicates are suppressed. This corrects the later healthy-session skills failure, which finalized the user request but produced only an acknowledgement and no native delegation. It is distinct from the older OpenClaw 2026.8.1 stale-request-root admission defect; `/startupz` monitors only global starting/draining state, retains cleanup controls, and now restores connected state if the same socket survives recovery. The ordinary Interrupt button is removed. Two sustained speech frames at the official desktop thresholds clear local playback and cancel the exact output turn; provider `clear` rejects late audio. A partial user transcript enters `Hearing you`, and a 12-second non-finalization watchdog ends the failed session cleanly instead of hanging. Source owner/lifecycle tests and isolated lint/type gates pass; account reauthentication and corrected Gateway restart are complete. Live proof still awaits a real three-skills result, natural barge-in, and second same-session result. Direct Hermes/Codex/Claude adapters remain voice-unsupported in the current implementation; an OpenClaw session consulting a similarly named model is still OpenClaw-owned and must not be mislabeled.
