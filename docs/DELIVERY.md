...
Next: close F5d.3 now that the refreshed Cosy Chiruka OAuth profile is first and `gpt-live-1-codex`/`spruce` session admission passes after the account switch: real assistant audio, transcript/output ordering, barge-in during audible playback, clear/mark timing, provider disconnect and same-session recovery. Then add reviewed device selection and truthful audio-driven facial response. Remote OpenClaw/Hermes deployment evidence, product-suitable CC0 VRM 1.0 selection, Windows external full-screen/manual accessibility/physical-display evidence, macOS, Claude, and production commerce remain owner/external gates.
