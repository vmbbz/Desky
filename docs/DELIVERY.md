...
F5b.1 Hermes foundation is implemented against official source revision `057dcdf236f8a6a26721c10fcc6ccb72726e272a`: authenticated capability and version admission, strict endpoint policy, bounded structured HTTP/SSE, sessions, streaming, tool/subagent lifecycle, per-run approvals, truthful stop, disconnect, redaction, and exactly-once terminal fixtures pass. F5b.2 now has executable opt-in admission and full-model live matrices. The provider-independent matrix passes against an installed loopback Hermes Agent `0.20.5`: wrong-bearer redaction, authenticated source/version/capability admission, and disposable session resources are real-server proved. The full command adds real streaming, approval deny/allow, Stop during actual execution, and terminal dedupe, but remains unrun because this machine has no separately authorized Hermes model provider. The adapter stays unregistered and `production: false`; authenticated full-model source/package evidence, bounded reconnect, credential persistence, packaged lifecycle, and typed Desky-action discovery remain open.
