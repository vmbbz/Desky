...
- A five-pixel threshold preserves normal click/double-click behavior. Pointer-down is raycast against the rendered avatar: direct mesh drag rotates the character, while transparent space inside the measured interaction bounds routes to native-window movement using absolute screen coordinates and work-area clamping. The focused grip is the guaranteed movement affordance.
- Shift-drag or Alt-drag forces avatar rotation even when raycasting misses. Mouse-wheel and Left/Right arrow rotation are equivalent, Home resets the view, and the normalized angle is persisted with desktop state.
...
The Energy setting changes decorative cadence and eligible semantic programs, never normalized state truth or safety priority. A separate persisted Movement setting controls the accessibility envelope: Full motion, Follow Windows, or Reduced motion. Per-avatar compatibility profiles may remove unsafe categories or programs regardless of either setting. Explicit typed user/agent actions remain deliberate and bounded; a lively preset does not authorize a gateway or model to select arbitrary clips.

This control is now implemented and persisted in main-owned desktop state. The UI calls the `paused` policy **Still**: it suppresses body motion through the existing reduced execution path without rewriting the separate Movement choice. Quiet admits only low Presence/Conversation/Reactions, disables Playful and Locomotion autonomy, and lengthens file-defined quiet windows. Balanced is the default; Lively shortens cadence only over admitted categories. Custom exposes bounded 0–3 category levels. Full motion is the default Movement policy; Follow Windows and Reduced motion remain explicit saved alternatives. The scheduler maps semantic tags to categories and still contains no filename list.
...
The candidate library supplies 84 canonical CC0 Quaternius Standard clips plus the separately licensed, transformed 11.4-second Mixamo Looking Around clip. State bindings, action/catalog programs, exact steps, tags, quiet windows, and cooldowns live in the generated asset file; the scheduler contains no animation filename list. Looking Around is the sole full-body owner in idle/disconnected, Search/Interact owns thinking, and the talk loop owns speaking. There is no folded-arm state or procedural body overlay fighting the idle. Those standing states preserve the target avatar hips height to prevent vertical drift. The controlled autonomous bag is Phone Check, Formal Walk, and rare Dance Break; it never selects across all 85 files. The old `Idle_No_Loop`/`Yes` cadence is no longer autonomous because it read as constant head shaking and thumbs-up, and Crouch/Search was rejected on Milk for limb distortion. Rich sit/chat, magic, crouch, and other edge cases remain catalog-only for future contextual requests and per-avatar visual admission. Authored steps play for their full canonical duration, adjacent steps crossfade without passing through the bind pose, and the final pose blends into the accepted state rather than snapping. Jump uses a two-file sequence; Wave stays procedural until a reviewed Wave asset exists. Speech visemes and representative binary VRM evidence remain open and must preserve the same ownership boundary. The full inventory is in `docs/ANIMATION-CATALOG.md`.
...
F3c.5 closes the response-presentation handoff. Completing a turn no longer stops the mixer: semantic success crossfades into the admitted Looking Around body loop, and error/disconnected states use the same living fallback when no exact clip is admitted. The small ambient bubble strips Markdown structure before bounded previewing and auto-dismisses successful replies after an 8–18 second reading interval. The control center retains the complete bounded response and renders headings, emphasis, lists, tables, code, quotes, and credential-free web links without executing raw HTML. **Open conversation** first asks a main-owned, argument-free provider router; an installed OpenClaw Windows Companion receives its documented `openclaw://chat` route, while unsupported, absent, or failed clients fall back to the synchronized Deskii transcript.

F3c.6 keeps asset provenance available without turning it into permanent ambient chrome. The ready-state model/version/licence summary remains in the control center and asset records; the companion surface shows only transient loading and actionable graphics/error recovery. Avatar hover or keyboard focus exposes a compact custom control guide for character drag rotation, transparent-gap movement, and jump instead of the operating system's unbounded native title tooltip. The guide is delayed, non-interactive, reduced-motion safe, and suppressed when a below-avatar response needs the same space.
...
The Windows implementation now stops the animation-frame loop for main-reported native hiding, Chromium-reported document hiding/occlusion, Electron power suspension, and WebGL loss. It uses active-only motion time so hidden intervals do not skip autonomous programs. A bounded unrecoverable-WebGL state keeps ordinary UI available and offers a fresh-canvas **Retry graphics** route. Main separately persists whether the companion should be visible; a two-second watchdog re-presents unexpected native hide/minimize with `showInactive`, while deliberate Hide and power suspend always win. Display bursts reconcile after 250 ms before clamping and persisting. Packaged evidence proves unexpected-hide and minimize recovery, deliberate-hide preservation, focus preservation, a real auto-woken Modern Standby cycle, withheld WebGL restoration followed by retry, 80 transactional avatar changes, and whole-app idle/thinking/hidden plateaus. Arbitrary overlap occlusion where Chromium does not report it, external full-screen observation, physical display/virtual-desktop matrices, manual assistive technology, and macOS remain open.

F3d.1 replaces display-refresh polling with a timer/RAF scheduler. Calm state loops target 20 FPS; autonomous gesture programs, explicit cues, thinking, working, approval, speaking, and previews remain 60 FPS. A timed packaged Windows probe verifies exact native-hidden frame stability and recovery. Whole-app visible idle improved from 4.107% to 3.103%, hidden CPU averaged 0.081%, and renderer memory stayed below 350 MB. The composed `<3%` target remains narrowly open rather than being weakened or relabelled.
