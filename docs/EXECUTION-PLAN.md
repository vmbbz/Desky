...
Status: F4x.1a–c now provide strict commerce contracts and transactional conformance plus the hosted API/repository boundary, rotating Ed25519 JWKS, OS-encrypted refresh rotation, PKCE/idempotent clean-device recovery, signed bounded offline lease, pinned offline verification key, trusted-time checks, and reconciliation-before-persist. F4x.2a–d pin the provider-disabled Base Sepolia x402 v2 exact/EIP-3009 boundary; add durable authorization, monotonic settlement observations, transaction uniqueness, retry/closure blocking, and settled-evidence-only grant; establish strict hosted checkout sessions and recoverable system-browser handoff; and implement the fragment challenge, `__Host-` cookie, CSRF, browser API client, EIP-1193/EIP-712 signing, digest/lease recovery, and authoritative status projection. Opening a page does not prepare payment and no signature is persisted. These modules remain unwired from Electron IPC and paid providers remain disabled. Remaining F4x.1 operations: deploy the real database/API/identity/TLS/key-custody stack, migrations/backups/restore, reconciliation workers/monitoring, multi-instance tests, and clean-device hardware evidence. Next F4x.2 work is the funded Base Sepolia matrix on a real hosted ingress/wallet, settled-to-grant projection, and clean-device purchase restoration; no production commerce is enabled.
