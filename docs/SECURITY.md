...
- The testnet checkout is a separately deployed Netlify application with hashed first-party assets, same-origin-only CSP, no inline/third-party execution, no framing/referrer, bounded streamed request bodies, and fixed Function routes. Hosted state uses PostgreSQL transactions and row locks; SQLite/process memory are never deployment authority. Missing database, merchant, or facilitator configuration returns sanitized 503. The current Netlify plan rejected database provisioning, so the live testnet origin remains deliberately non-payable.
