...
The autonomous scheduler is deliberately bounded rather than an endless action loop. State clips remain continuous where their file contract says `loop`; one reviewed program is then selected after its file-defined quiet interval. A validated cadence can assign exact cycle slots, while uncadenced libraries retain weighted, cooldown-aware selection with immediate-repeat exclusion. A companion mode cannot silently mix the policies or declare incomplete/competing cycles. The current desktop baseline is the exact 11.4-second Looking Around clip, including while disconnected, so connectivity can never expose an imported bind pose. There is one full-body idle owner: `Idle_FoldArms_Loop` is catalog-only and the former procedural look overlay is removed. Weighted ambient programs admit only Phone Check, in-place Formal Walk, and rare Dance Break. The weighted pool behaves as a shuffled bag until every admitted program has appeared once, so a favourite cannot hide the rest of the repertoire. `Idle_No_Loop` and `Yes` remain catalog-only because packaged review showed that their former cadence appeared as repetitive head motion and thumbs-up. Crouch/Search is also catalog-only after Milk-specific visual review exposed unacceptable limb stretching. Entering a meaningful state, local preview, reduced motion, or an explicit action resets the quiet interval. Code contains timing, priority, validation, interruption, transition, and accessibility policy—not animation filenames.
...
F3d.2 adds a second, avatar-specific admission boundary. Every marketplace revision now names an immutable animation profile. `desky-humanoid-standard-v1.profile.json` owns the exact library, required humanoid bones, state modes, runnable semantic programs, reviewed intensity, root-motion prohibition, reviewer, and review timestamp. Runtime requires the profile and library to own exactly the same non-catalog programs and state bindings. A catalog-only clip cannot become runnable merely by changing its library trigger, an unprofiled avatar cannot render, and a required-bone mismatch fails before the mixer starts. The controller receives only the four profile-enabled programs; the other eleven catalog programs and their clips remain inspectable data, not executable runtime registrations.

Intensity is part of the reviewed profile rather than inferred from an animation filename. Autonomous scheduling requires the user's semantic category level to meet that program's reviewed intensity. Explicit typed actions remain governed by the action/approval boundary, not the decorative personality setting. Root motion is prohibited for the current desktop profile and rechecked against every enabled program manifest.
...
The renderer subscribes to `prefers-reduced-motion`. In reduced-motion mode it suppresses registered full-body clips, removes time-varying travel and bounce (including Jump translation), neutralizes gaze, and retains only a small static semantic/action acknowledgement plus the readable state label. The control center exposes one durable Movement policy beside Companion energy: `Full motion` is the product default, `Follow Windows` delegates to the operating-system preference, and `Reduced motion` is an explicit stricter choice. Selecting Full while Windows requests reduced motion produces an explicit notice; the override is never implied by an unrelated preview tool. `Still` remains the independent zero-energy temperament and does not rewrite the saved accessibility envelope.
