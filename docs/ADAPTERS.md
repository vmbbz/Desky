...
- Runtime status: the unregistered `CodexRuntime` now passes fixtures for account admission, thread list/start/resume, turn start, assistant streaming, paired redacted tools, command/file approvals, cancellation, exactly-one terminal events, disconnect, and unexpected process exit. A live installed-CLI smoke passed initialization, account read, and thread listing without starting a model turn. It remains `production: false`, unregistered, and reconnect/action unavailable.
- Remaining admission sequence: generated-schema artifact pipeline and validator coverage for every consumed native shape; approved workspace picker and sandbox disclosure; restart/reconnect; typed actions only through a supported stable tool surface; direct-profile UI; packaged authenticated model/tool/approval/cancellation matrix. WebSocket is excluded because the official current surface labels it experimental and unsupported for production.
